 ...
 *
 * auton program based on the front position for the red team that uses
 * encoders for longer distances and rotation. For 8553
 *
 */
@Autonomous(name = "Red Front 8553", group = "Linear Opmode")
//@Disabled
public class Auton_Red_Front8553 extends LinearOpMode {
    ...
    private static final double DISTANCE_FOR_LEFT_COLUMN = ( 110.81 - 2.54 );    //
    private static final double DISTANCE_FOR_CENTER_COLUMN = ( 91.44 - 2.54 );   //
    private static final double DISTANCE_FOR_RIGHT_COLUMN = ( 72.07 - 2.54 );    //
            ...
            switch ( step )
            {
                ...
                case KNOCK_OFF_JEWEL:
                    ...
                    if ( color == JewelKnocker.COLORS.BLUE )
                    {
                        ...
                        // means distance from start is negative (away from crypto box).
                ...
                case MOVE_IN_FRONT_OF_BOX:
                {
                    ...
                    {
                        case LEFT:
                            errorTicks = go.AutonReverse( DISTANCE_FOR_LEFT_COLUMN - yDistanceFromStart );
                            rotateAdjustment = -5.0;
                            break;

                        case RIGHT:
                            errorTicks = go.AutonReverse( DISTANCE_FOR_RIGHT_COLUMN - yDistanceFromStart );
                            rotateAdjustment = 5.0;
                            break;

                        default:  // Default is for unknown or center
                        {
                            errorTicks = go.AutonReverse( DISTANCE_FOR_CENTER_COLUMN - yDistanceFromStart );
